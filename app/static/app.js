...
function renderMateTrocaPendingHistoryInDialog(events, cod, productName) {
  ...
  const parts = sorted.map((ev) => {
    ...
    if (kindRaw === 'chegada') {
      ...
      mov = `Na tela Quebra, para aquele dia, ficou anotado: CX ${formatBreakIntegerBR(ev.qty_cx_in)} · UN ${formatBreakIntegerBR(ev.qty_un_in)} (total do dia na base).`;
...
/** Texto compacto mobile. Troca: sempre CX e UN; quebra: só dimensões ≠ 0. */
...
/** Uma linha no detalhe (métricas). Troca: sempre CX e UN; quebra: só partes ≠ 0. */
...
function _biQuebrasEnsureLayout() {
  /* Markup estático em index.html (tema count-audit). */
}
...
function _biQuebrasResizeChartsSoon() {
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      ...
      try { _biQuebrasChartSegmento?.resize(); } catch { /* ignore */ }
    });
  });
}
...
function _biQuebrasRenderKpisClassic(data) {
  ...
  _biQuebrasSetText(
    'bi-quebras-kpi-period',
    activeDays.length
      ? `${period} · ${_biQuebrasScopeLabel(_biQuebrasSelectedCiaScope)} · ${activeDays.length} dia(s) com lançamento`
      : `${period} · ${_biQuebrasScopeLabel(_biQuebrasSelectedCiaScope)} · sem lançamento`,
  );
  _biQuebrasSetText('bi-quebras-kpi-products-note', 'produtos únicos');
  _biQuebrasSetText('bi-quebras-kpi-cx-note', 'caixas quebradas');
  _biQuebrasSetText('bi-quebras-kpi-un-note', 'unidades quebradas');
  _biQuebrasSetText(
...
function _biQuebrasRenderSegmentoChart(companies) {
  const canvas = document.getElementById('bi-quebras-chart-segmento');
  if (!canvas) return;
  const slice = _biQuebrasNormalizeCompanies(companies).slice(0, 10);
  const withLoss = slice.filter((row) => (Number(row.loss_brl) || 0) > 0);
  if (_biQuebrasChartSegmento) {
    _biQuebrasChartSegmento.destroy();
    _biQuebrasChartSegmento = null;
  }
  if (!withLoss.length || typeof Chart !== 'function') {
    return;
  }
  _biQuebrasChartSegmento = new Chart(canvas, {
    type: 'doughnut',
    data: {
      labels: withLoss.map((row) => String(row.cia || '--')),
      datasets: [{
        data: withLoss.map((row) => Number(row.loss_brl) || 0),
        backgroundColor: BI_QUEBRAS_PALETTE.slice(0, withLoss.length),
        borderWidth: 2,
        borderColor: '#ffffff',
        hoverOffset: 8,
      }],
...
function _biQuebrasRenderSegmentoList(companies) {
  const list = document.getElementById('bi-quebras-segmento-list');
  if (!list) return;
  const items = _biQuebrasNormalizeCompanies(companies);
  const sel = String(_biQuebrasSelectedCompany || '').trim();
  if (!items.length) {
    list.innerHTML = '<li><div class="bi-quebras-breakdown-empty">Nenhuma CIA com impacto no período.</div></li>';
    return;
  }
  list.innerHTML = items
    .map((row, i) => {
      const cia = String(row.cia || '--').trim();
      const loss = Number(row.loss_brl) || 0;
      const pct = row.pct;
      const isSel = sel && sel === cia;
      const dot = BI_QUEBRAS_PALETTE[i % BI_QUEBRAS_PALETTE.length];
      return `
      <li>
        <button type="button" class="bi-quebras-breakdown-row bi-quebras-breakdown-row--interactive${isSel ? ' is-selected' : ''}" data-bi-quebras-company="${_biQuebrasEscapeHtml(cia)}" aria-pressed="${isSel ? 'true' : 'false'}">
          <span class="bi-quebras-bd-dot" style="background:${dot}" aria-hidden="true"></span>
          <span class="bi-quebras-bd-label">${_biQuebrasEscapeHtml(cia)}</span>
          <span class="bi-quebras-bd-items">${_biQuebrasFormatNumber(row.items)}</span>
          <span class="bi-quebras-bd-pct">${pct != null ? _biQuebrasFormatPct(pct) : '—'}</span>
          <span class="bi-quebras-bd-loss">${_biQuebrasFormatBRL(loss)}</span>
          <span class="bi-quebras-bd-occ">${_biQuebrasFormatNumber(row.total_cx)}/${_biQuebrasFormatNumber(row.total_un)}</span>
        </button>
      </li>`;
    })
    .join('');
...
function _biQuebrasRenderReasonListClassic(items) {
  const list = document.getElementById('bi-quebras-reason-list');
  if (!list) return;
  const rows = _biQuebrasGetReasonItems(items).slice(0, 20);
  if (!rows.length) {
    list.innerHTML = '<li><div class="bi-quebras-breakdown-empty">Nenhum motivo com prejuízo estimado no período.</div></li>';
    return;
  }
  list.innerHTML = rows
    .map((row, i) => {
...
function _biQuebrasRenderCompanyProductsPanel() {
  ...
  const metaEl = document.getElementById('bi-quebras-company-products-meta');
  if (!list) return;
  const sel = String(_biQuebrasSelectedCompany || '').trim();
  const company = _biQuebrasCurrentCompanies.find((item) => String(item.cia || '').trim() === sel);
  if (!sel || !company) {
    if (titleEl) titleEl.textContent = 'Produtos da CIA';
    if (metaEl) {
      metaEl.textContent = 'Clique em uma CIA na lista ao lado para ver os produtos do grupo.';
    }
    if (countEl) countEl.textContent = '0';
    list.innerHTML = '<li class="bi-quebras-company-products-empty">Clique em uma CIA na lista para ver os produtos.</li>';
    return;
  }
  if (titleEl) titleEl.textContent = `Produtos — ${String(company.cia || '—')}`;
  if (metaEl) {
    metaEl.textContent = `${_biQuebrasFormatNumber(company.total_cx)} CX · ${_biQuebrasFormatNumber(company.total_un)} UN · ${_biQuebrasFormatNumber(company.items)} produtos`;
  }
  ...
  list.innerHTML = filteredProducts
    .map((product, index) => {
      ...
      <li class="bi-quebras-company-product-row" role="listitem"${rowTip ? ` title="${_biQuebrasEscapeHtml(rowTip)}"` : ''}>
        ...
        <span class="bi-quebras-company-product-loss${lossExtra}">${_biQuebrasFormatBRL(lv)}</span>
      </li>`;
    })
    .join('');
}
...
function _biQuebrasSelectCompany(cia) {
  ...
  if (!normalized) return;
  if (_biQuebrasSelectedCompany === normalized) {
...
function _biQuebrasClearCompanySelection() {
  _biQuebrasSelectedCompany = '';
  _biQuebrasRenderSegmentoList(_biQuebrasCurrentCompanies);
  _biQuebrasRenderCompanyProductsPanel();
...
async function loadBiQuebras() {
  ...
  _biQuebrasRenderDashboard(data);
}
...
function bindBiQuebrasEvents() {
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (_biQuebrasSelectedCompany) {
      event.preventDefault();
      _biQuebrasClearCompanySelection();
      return;
    }
